...
- 感受野（Receptive Field）是 CNN 中每个神经元在输入图像上的可视范围。
- 较小的感受野用于捕捉细节信息，而较大的感受野可以学习全局特征。
- 通过增加层数、使用池化层或膨胀卷积，可以有效增大感受野，使模型能学习更丰富的特征。
...
| **问题**            | **核心知识点**                           | **代码示例**                    |
| ------------------- | ---------------------------------------- | ------------------------------- |
| **CNN 层**          | 卷积层提取特征，池化层降维，全连接层分类 | `nn.Conv2d()`                   |
| **ML vs. DL**       | 深度学习适合大数据，机器学习适合小数据   | `RandomForestClassifier vs. NN` |
| **BN 层**           | 让训练更稳定，加速收敛                   | `nn.BatchNorm2d()`              |
| **超参数**          | 影响收敛速度和泛化能力                   | `optimizer.Adam(lr=0.001)`      |
| **Batch Size 影响** | 大 batch 震荡小，小 batch 泛化好         | `DataLoader(batch_size=32)`     |
| **Dropout**         | 随机丢弃神经元，减少过拟合               | `nn.Dropout(0.5)`               |
| **感受野**          | 计算神经元在输入图像上的影响范围         | `compute_rf(3)`                 |
