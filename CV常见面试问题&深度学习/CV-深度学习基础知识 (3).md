...
- **🔥 Fire Module：使用 "Squeeze" + "Expand" 结构**
- **💡 1×1 卷积：减少参数，提高计算效率**
- **🔄 没有全连接层（FC）：减少计算量**
...
| **特点**       | **解释**                                   |
| -------------- | ------------------------------------------ |
| **减少参数量** | Fire Module 让网络更轻量级                 |
| **更少计算量** | 适用于嵌入式系统和移动设备                 |
| **高效推理**   | 计算更快，适用于实时任务                   |
| **保留精度**   | 通过 Fire Module 维持 AlexNet 级别的准确率 |
...
CNN 之所以强大，主要因为它的 **卷积核（Convolutional Kernel）** 具备以下四个核心特性：

1. **平移不变性（Translation Invariance）**
2. **平移等变性（Equivariance）**
3. **尺度不变性（Scale Invariance）**
4. **旋转不变性（Rotation Invariance）**
...
- **CNN 处理一张图片，无论物体位于左边还是右边，它都能正确识别！**
- 例如，一个 **猫的图片**，无论**猫在左上角还是右下角，CNN 都能识别出它是猫**。

📌 **为什么 CNN 具备平移不变性？**

- 因为 CNN 使用 **池化层（Pooling）**，它能捕捉最重要的特征，而不会受到物体位置的影响。
...
x = torch.tensor([[1, 2], [3, 4]], dtype=torch.float32).reshape(1, 1, 2, 2)
print(F.max_pool2d(x, kernel_size=2))  # 只保留最大值，消除位置信息
```
...
- 平移等变性指的是，**如果输入平移，Feature Map 也会相应地平移**。
- 也就是说，**CNN 会找到图片中的物体，并在相应的位置生成激活响应**，即 CNN 只关心**相对位置**。
...
- **物体变大或变小，CNN 仍然能识别它。**
- 例如：
  - <u>小猫 🐱 和 大猫 🦁，CNN 都能识别是猫！</u>
- 但是 CNN **并不能完全做到尺度不变**，如果物体缩放过大或过小，CNN 可能无法识别。

📌 **解决方案**

- 采用 **金字塔尺度（Image Pyramid）**，在多个尺度上训练 CNN。
- 例如，在 Faster R-CNN 目标检测网络中，会使用 **特征金字塔（FPN）** 处理不同尺度的物体。
...
resize = transforms.Resize((128, 128))  # 统一缩放到 128×128
```
...
- **物体旋转后，CNN 仍然能够识别它**。
- 例如：
  - 🐱（正常） vs. 🔄🐱（旋转 90°） → CNN 仍然识别为 "猫"。
- 但 CNN 对旋转不敏感，旋转角度过大时，CNN 可能无法识别。

📌 **解决方案**

- **数据增强（Data Augmentation）**，对训练数据进行随机旋转，让 CNN 适应旋转后的物体。
- 例如，在 ImageNet 训练时，使用 `transforms.RandomRotation()`。

🔧 **代码示例**

```python
rotate = transforms.RandomRotation(degrees=30)  # 让图片随机旋转 30 度
```
...
| **特性**       | **作用**                         |
| -------------- | -------------------------------- |
| **平移不变性** | 物体在不同位置 CNN 仍然能识别    |
| **平移等变性** | 卷积核找到特征，不受位置信息影响 |
| **尺度不变性** | 物体变大或变小，CNN 仍然能识别   |
| **旋转不变性** | 物体旋转后，CNN 仍然能识别       |
...
| **CNN 特性**                       | **作用**                           |
| ---------------------------------- | ---------------------------------- |
| **局部连接（Local Connectivity）** | 只关注局部特征，减少计算量         |
| **权重共享（Weight Sharing）**     | 相同卷积核扫描整个图像，减少参数量 |
| **层级特征学习**                   | 学习从低级到高级的特征             |
| **平移不变性**                     | 物体在不同位置 CNN 仍然能识别      |
| **自动提取特征**                   | 不需要手工设计特征                 |
| **空间层次结构**                   | 逐步减少特征图尺寸，提高计算效率   |
| **端到端学习**                     | 直接从原始数据学习特征，进行分类   |
